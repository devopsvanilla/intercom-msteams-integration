...
class IntercomClient:
        ...
        try:
            async with self.session.request(method, url, json=data) as response:
                ...
                if response.status >= 400:
                    logger.error(
                        f"Intercom API error {response.status}: {response_data}"
                    )
                    error_msg = response_data.get("errors", [{}])[0].get(
                        "message", "Unknown error"
                    )
                    raise Exception(f"Intercom API error: {error_msg}")
    ...
    async def get_conversations(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self._make_request(
                "GET", f"/conversations?per_page={limit}"
            )
    ...
    async def create_conversation(
        self, user_id: str, body: str, message_type: str = "comment"
    ) -> Dict[str, Any]:
        """
    ...
    async def reply_to_conversation(
        ...
        """
        try:
            data = {"message_type": message_type, "body": body}

            if admin_id:
                data["admin_id"] = admin_id

            response = await self._make_request(
                "POST", f"/conversations/{conversation_id}/reply", data
            )
    ...
    async def get_conversation_parts(
        ...
        try:
            ...
            parts = conversation.get("conversation_parts", {}).get(
                "conversation_parts", []
            )

            logger.info(
                f"Retrieved {len(parts)} parts for conversation {conversation_id}"
            )
            return parts

        except Exception as e:
            logger.error(
                f"Failed to get conversation parts for {conversation_id}: {str(e)}"
            )
            raise

    async def trigger_fin_ai_response(
        ...
        try:
            ...
            response = await self._make_request(
                "POST", "/conversations/ai/suggest", data
            )
            ...
            logger.warning(
                f"FIN AI response not available for conversation "
    ...
    async def create_or_update_user(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            user_identifier = user_data.get("email", user_data.get("user_id"))
            logger.info(f"Created/updated user {user_identifier}")
            return response
