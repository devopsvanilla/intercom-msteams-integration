...
def mock_graph_client():
    ...
    client.get_team_channels = AsyncMock(
        return_value=[
            {
                "id": "channel1",
                "displayName": "General",
    ...
    client.find_or_create_channel = AsyncMock(
        return_value={"id": "channel1", "displayName": "Customer Support"}
    )
    return client
...
class TestGraphClient:
    ...
    async def test_authentication_success(self):
        ...
        with (
            patch("graph_client.ClientSecretCredential"),
    ...
    async def test_send_message_success(self, mock_graph_client):
        ...
        with (
            patch("graph_client.ChatMessage"),
            patch("graph_client.ItemBody"),
            patch("graph_client.BodyType"),
        ):
            mock_post = mock_graph_client.client.teams.by_team_id()
            mock_post = mock_post.channels.by_channel_id().messages.post
            mock_post = AsyncMock(
                return_value=Mock(
                    id="msg1",
            ...
            messages_endpoint = mock_graph_client.client.teams.by_team_id()
            messages_endpoint = messages_endpoint.channels.by_channel_id().messages
            messages_endpoint.post = mock_post
...
class TestIntercomClient:
    ...
    async def test_get_conversations_success(self):
        ...
        mock_response.json = AsyncMock(
            return_value={
                "conversations": [
                    {"id": "conv1", "source": {"author": {"name": "Test"}}}
                ]
            }
        )
...
class TestWebhookHandler:
    ...
    def test_verify_webhook_signature_invalid(self):
        ...
        is_valid = handler.verify_webhook_signature(
            payload, f"sha256={invalid_signature}"
        )
        assert is_valid is False

    @pytest.mark.asyncio
    async def test_process_webhook_conversation_created(
        ...
        with patch.object(
            handler, "_handle_conversation_created", return_value={"status": "success"}
        ):
            result = await handler.process_webhook(
                "conversation.user.created", {"data": {"item": {"id": "conv1"}}}
            )

            assert result["status"] == "success"

    @pytest.mark.asyncio
    async def test_process_webhook_unknown_event(
        self, mock_graph_client, mock_intercom_client
    ):
        ...
        result = await handler.process_webhook(
            "unknown.event", {"data": {"item": {"id": "conv1"}}}
        )
